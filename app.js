...
function portfolioMetrics(){
  if(!localStorage.getItem('fxcc-trades')) return {equity:48260.40,netProfit:8260.40,maxDrawdown:8.42,activeEas:4};
  let curve=0,peak=0,maxDrop=0; state.trades.forEach(t=>{curve+=Number(t.profit)||0;peak=Math.max(peak,curve);maxDrop=Math.max(maxDrop,peak-curve)});
  return {equity:40000+curve,netProfit:curve,maxDrawdown:maxDrop/482.6,activeEas:currentAnalysisData().length};
}
function renderPortfolioSummary(){
  const metrics=portfolioMetrics();$('#portfolio-equity').textContent=formatMoney(metrics.equity);$('#net-profit').textContent=formatSignedMoney(metrics.netProfit);$('#max-drawdown').textContent=`-${metrics.maxDrawdown.toFixed(2)}%`;$('#active-eas').innerHTML=`${String(metrics.activeEas).padStart(2,'0')} <small>/ ${metrics.activeEas}</small>`;
}
...
function renderHealthList(){
  const source=currentAnalysisData();$('#health-list').innerHTML=source.map(e=>`<article class="health-row"><div><h3>${e.name}</h3><p>${e.symbol} · ${e.note}</p></div><div class="health-metric"><span>STATUS</span>${statusTag(e.status)}</div><div class="health-metric"><span>UPTIME</span><strong>${e.uptime}</strong></div><div class="health-metric"><span>HEARTBEAT</span><strong class="${e.status==='offline'?'negative':''}">${e.heartbeat}</strong></div><div class="health-metric last-trade"><span>LAST P/L</span><strong class="${e.pnl>=0?'positive':'negative'}">${formatSignedMoney(e.pnl)}</strong></div><button class="action-button" data-health-ea="${e.id}">詳細</button></article>`).join('');
  const counts=source.reduce((a,e)=>(a[e.status]++,a),{healthy:0,watch:0,offline:0});$('#health-healthy').textContent=String(counts.healthy).padStart(2,'0');$('#health-watch').textContent=String(counts.watch).padStart(2,'0');$('#health-offline').textContent=String(counts.offline).padStart(2,'0');
}
...
function updateLab(){
  ...
  const metrics=portfolioMetrics(),baseProfit=metrics.netProfit||1,baseDd=metrics.maxDrawdown||.1,baseSharpe=1.42,multiplier=lot*(1+win/100)*(1-spread/250),horizon=period/30,profit=baseProfit*multiplier*horizon,dd=baseDd*lot*(1+spread/180)-win*.08,sharpe=Math.max(.2,baseSharpe*(1+win/100)/(lot*.72+0.28));
  $('#projected-profit').textContent=formatSignedMoney(profit);$('#profit-delta').textContent=`${profit>=baseProfit?'+':''}${((profit/baseProfit-1)*100).toFixed(1)}% vs baseline`;$('#profit-delta').className=profit>=baseProfit?'positive':'negative';$('#projected-dd').textContent=`-${dd.toFixed(2)}%`;$('#dd-delta').textContent=`${dd<=baseDd?'+':''}${(baseDd-dd).toFixed(2)}pt vs baseline`;$('#dd-delta').className=dd<=baseDd?'positive':'negative';$('#projected-sharpe').textContent=sharpe.toFixed(2);$('#sharpe-delta').textContent=`${sharpe>=baseSharpe?'+':''}${(sharpe-baseSharpe).toFixed(2)} vs baseline`;
...
function promptText(){
  ...
  const metrics=portfolioMetrics();const text=`あなたは経験豊富なFXシステムトレードのリサーチャーです。以下のデータをもとに「${title}」を日本語で行ってください。\n\n【目的】\n${title}について、事実と推測を分けて、実行可能な提案をしてください。過度な断定や利益保証は避けてください。\n\n【ポートフォリオ概要】\nEquity: ${formatMoney(metrics.equity)}\nNet Profit: ${formatSignedMoney(metrics.netProfit)}\nMax Drawdown: -${metrics.maxDrawdown.toFixed(2)}%\nActive EAs: ${metrics.activeEas}\nTrade count: ${state.trades.length}\n\n【EA別パフォーマンス】\n${perf}\n\n【直近の取引】\n${state.trades.slice(0,5).map(t=>`- ${t.date} | ${t.ea} | ${t.symbol} ${t.type} ${t.lots} lots | ${formatSignedMoney(Number(t.profit))}`).join('\n')}\n\n【回答フォーマット】\n1. エグゼクティブサマリー（3行以内）\n2. 重要な観察（数値を引用）\n3. リスク・注意点\n4. 次の7日間で試すこと（優先順位付き）\n5. 追加で必要なデータ\n${context?`\n【ユーザーからの追加質問】\n${context}`:''}`;
...
function init(){
  ...
  $('#mobile-menu').addEventListener('click',()=>$('#sidebar').classList.toggle('open'));$('#reset-data').addEventListener('click',()=>{state.trades=[...demoTrades];localStorage.removeItem('fxcc-trades');localStorage.removeItem('fxcc-imported-at');renderTrades();renderAnalysis();renderHealthTable();renderHealthList();renderPortfolioSummary();updateDataset();promptText();toast('デモデータに戻しました')});
  $('#choose-file').addEventListener('click',()=>$('#csv-file').click());$('#csv-file').addEventListener('change',e=>processFile(e.target.files[0]));$('#download-sample').addEventListener('click',sampleCsv);$('#clear-data').addEventListener('click',()=>{localStorage.removeItem('fxcc-trades');localStorage.removeItem('fxcc-imported-at');state.trades=[...demoTrades];renderTrades();renderAnalysis();renderHealthTable();renderHealthList();renderPortfolioSummary();updateDataset();promptText();toast('保存データをクリアしました')});
  ...
  renderHealthTable();renderAnalysis();renderHealthList();renderTrades();renderPortfolioSummary();updateDataset();drawEquity();calculateRisk();updateLab();promptText();$('#last-updated').textContent=new Date().toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'});setView(location.hash.slice(1)||'overview');
}
